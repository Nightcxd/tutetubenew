...
public class NetHelper {
	...
	public PersonalInfo getPersonalInfo(UserData uData) {
		try {
			...
			Document doc = Jsoup.connect("http://202.113.244.44:9013/reportFiles/student/cj_zwcjd_all.jsp")
		...
		try {
			...
			for (Element tblView_line : tblView_tr)
			{
				...
				if (tmpStringKey.length() != 0) {
					tmpMap = new HashMap<String, String>();
					tmpMap.put("key", tmpStringKey.trim());
					tmpMap.put("value", tmpStringValue.trim());
					rollList.add(tmpMap);
				}
	...
	public String changePassword(UserData uData,String oldPass, String newPass1,String newPass2) {
		try {
			...
			Connection con = Jsoup.connect("http://202.113.244.44:9013/modifyPassWordAction.do?oper=xgmm")
					.cookie("JSESSIONID", session)
					.data("yhlbdm", "01")
	...
	public ArrayList<Map<String, String>> examInfo(UserData uData)
	{
		...
		try
		{
			...
			/** 联网，连接中包含Cookie信息 **/
			Document doc = Jsoup.connect("http://202.113.244.44:9013/ksApCxAction.do?oper=getKsapXx")
					.cookie("JSESSIONID", session)
					.timeout(10000)
					.get();
	...
	public List<Map<String, String>> getGrades(UserData uData) {
		try {
